...
from monai.transforms import Compose, LoadImaged, AddChannel, ScaleIntensityd, ToTensor,EnsureChannelFirstd
...
data_dicts = [{"image": image_name, "label": label_name} for image_name, label_name in zip(train_image_dir_t1, train_image_dir_t2)]
train_files, val_files = data_dicts[:-9], data_dicts[-9:]

train_data = Dataset(data=data_dicts, transform=Compose([LoadImaged(keys=["image", "label"],image_only=True), EnsureChannelFirstd(keys=["image", "label"]), ScaleIntensityd(keys=["image", "label"])]))
...
class ContrastConversionModel(nn.Module):
    def __init__(self):
        ...
        self.unet = UNet(dimensions=3, in_channels=1, out_channels=1, channels=(16, 32, 64, 128), strides=(2, 2, 2), num_res_units=2, norm=Norm.BATCH)
